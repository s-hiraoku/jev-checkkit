...
製品チェックリストはまだ承認されていない。下の5件は対象の単位を決めるための候補である。項目そのものは、設計スキルが案を出し、利用者が承認してから定義にする。
...
最初の製品対象の有力な候補である。頻度が高く、既存のレビュー手順と比べて増えた価値を測りやすい。誤判定の影響は、レビュー議論をずらすことまでである。マージは人が決める。
...
このリポジトリの構想自体が引継ぎから始まっている。同じ形の文書を検査できると、計画と運用が一致する。誤判定の影響は、次の人の手戻りである。削除や外部送信はしない。
...
設計スキル自身を後からこの対象で検査できる。メタ文書向けの定義を先に製品化しない。誤判定の影響は、エージェントが古い手順を信じることである。
...
PR 説明より読者が広く、誤りのコストが高い。最初の精度検証には向かない。PR 説明でランナーの価値が見えてから定義する。公開は人が決める。
...
欠落を早く見せる用途である。自動で障害票を閉じたり、外部へ送ったりしない。誤判定の影響は、対応の優先度を間違えることである。最初の対象にはしない。実例と承認が先である。

## 採用しない用途

- 一般的な文章添削。Jev は校正器ではない。
- 構文、型、テスト、カバレッジ。コードで足りる判定を Jev に出さない。
- 「コードの品質」を1問で点数化すること。公式の勧めどおり、狭い質問に分解してコードで合成する。
- レポートを見てマージ、削除、公開、外部送信すること。閾値を通っても自動では行わない。
- チケット分類やモデレーションの汎用基盤。TypeSafe の用例にはあるが、このリポジトリの最初の価値ではない。
